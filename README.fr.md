<p align="center">
	<img src="https://raw.githubusercontent.com/Wing9897/ipscan/main/assets/banner.svg" alt="ipscan banner" width="100%" />
</p>

<div align="center">

# ipscan

Scanner IP rapide — Ping & ARP multithread (Windows)

[![PyPI](https://img.shields.io/pypi/v/ipscan?logo=pypi&label=PyPI)](https://pypi.org/project/ipscan/)
![Python](https://img.shields.io/pypi/pyversions/ipscan?logo=python)
![OS](https://img.shields.io/badge/OS-Windows%20%7C%20Linux%20%7C%20macOS-blue)
![Licence](https://img.shields.io/github/license/Wing9897/ipscan?color=success)

Langue :
[English](README.md) · [繁體中文](README.zh-TW.md) · [简体中文](README.zh-CN.md) · [日本語](README.ja.md) · [한국어](README.ko.md) · [Deutsch](README.de.md) · [Français](README.fr.md) · [Italiano](README.it.md) · [Español](README.es.md) · [Português BR](README.pt-BR.md) · [Русский](README.ru.md)

</div>

---

## Sommaire

- Démarrage rapide
- Fonctionnalités
- Outils CLI
- API Python
- Performances
- Prérequis
- Contribution

---

## Démarrage rapide

Installer via PyPI :

```bash
pip install ipscan
```

### CLI

```bash
fping           # ping continu haute vitesse (interactif)
sping           # scan de plage par ping
sarp            # scan de plage ARP
```

### API Python

Ping :

```python
from ipscan import ping_range, PingScanner

online_hosts = ping_range("192.168.1.1", "192.168.1.254")

scanner = PingScanner(timeout=1.0)
results = scanner.scan_range("10.0.0.1", "10.0.0.100")
```

ARP :

```python
from ipscan import arp_range, ArpScanner

host_info = arp_range("192.168.1.1", "192.168.1.254")
for ip, mac in host_info.items():
		print(f"{ip} -> {mac}")

scanner = ArpScanner()
results = scanner.scan_range("10.0.0.1", "10.0.0.100")
```

## Fonctionnalités

- Multithread pour une grande vitesse
- Ping et ARP, API simple
- Barre de progression, sortie claire
- Peu de dépendances

## Configuration Linux (recommandé)

Pour de meilleures performances sous Linux, accordez les permissions raw socket à Python :

```bash
# Activer le scan Ping rapide (raw ICMP socket, ~100x plus rapide)
sudo setcap cap_net_raw+ep $(readlink -f $(which python3))

# Activer le scan Ping + ARP rapide
sudo setcap cap_net_raw,cap_net_admin+ep $(readlink -f $(which python3))
```

Sans cette configuration :
- **Scan Ping** fonctionne mais utilise subprocess (plus lent)
- **Scan ARP** nécessite `sudo`

Restaurer (supprimer la permission) :
```bash
sudo setcap -r $(readlink -f $(which python3))
```

> **Note** : À faire une seule fois par installation Python. Pour un virtualenv, exécuter sur le binaire Python du venv.

## Performances

- Ping : /24 en quelques secondes (selon le matériel)
- ARP : très rapide sur le LAN

## Prérequis

- Python 3.7+
- Windows / Linux / macOS pris en charge ; sous Linux/macOS, ARP s'appuie sur les outils système (ip/arp/arping)

## Contribution

Issues & PR bienvenus. Une ⭐ est appréciée.

---

<div align="center">
Made with ❤️ for network tinkerers.
</div>
